"""Final push: extreme accuracy approaches to hit 0.995+.

Current best: s8 eps=0.10 me=0.15 → 0.9907 avg.

This tests:
  A) Raw polygon (skip curve fitting) at 4×/8× — maximum geometric fidelity
  B) Process ALL clusters (don't skip background) 
  C) SVG feGaussianBlur filter for built-in antialiasing
  D) Combined: s8 + core=0.45 + eps=0.08
  E) Even higher scale (12×, 16×) with tight fitting
  F) Optimized: s8 + core=0.45 + eps=0.08 + sc=0.4 ss=0.8
"""
...
def precompute(image_bgr, num_k=24, merge_thresh=60.0):
    ...
    soft_fields = {}
    for ci in range(K):  # ALL clusters
        d_k = dist_map[:, :, ci]
    ...
    return dict(h=h, w=w, K=K, bg_hex=bg_hex, bg_cluster=bg_cluster,
                ...
                soft_fields=soft_fields, bg_color=bg_color)
...
def run_v1(pre, *, scale=4, sc_sigma=0.6, ss_sigma=1.5,
           ...
           use_raw_polygon=False, svg_blur=0):
    ...
    for ci in pre['order']:
        ...
        for iso, op in zip(iso_levels, iso_opacities):
            ...
            for c in contours:
                ...
                if use_raw_polygon:
                    # Skip curve fitting: use raw polygon from marching squares
                    # Subsample every nth point to keep SVG size reasonable
                    step = max(1, len(xy) // 200)  # ~200 vertices max per contour
                    pts = xy[::step]
                    if len(pts) < 3:
                        continue
                    p = [f"M{pts[0,0]:.2f},{pts[0,1]:.2f}"]
                    for pt in pts[1:]:
                        p.append(f"L{pt[0]:.2f},{pt[1]:.2f}")
                    p.append("Z")
                    parts.append("".join(p))
                else:
    ...
    if svg_blur > 0:
        # Insert SVG filter for antialiasing
        blur_filter = (
            f'<defs><filter id="aa" filterUnits="userSpaceOnUse" '
            f'x="0" y="0" width="{w}" height="{h}">'
            f'<feGaussianBlur stdDeviation="{svg_blur}"/>'
            f'</filter></defs>'
        )
        svg = svg.replace(
        ...
        svg = svg.replace(' fill-rule="evenodd"', ' fill-rule="evenodd" filter="url(#aa)"')
...
if __name__ == "__main__":
    ...
    configs = [
        ...
        # Previous best
        ("s8 eps=0.10 me=0.15",
         dict(scale=8, eps=0.10, max_error=0.15)),

        # A) Raw polygon at different scales
        ("s4 raw_polygon",
         dict(scale=4, use_raw_polygon=True)),
        ("s8 raw_polygon",
         dict(scale=8, use_raw_polygon=True)),

        # B) Don't skip background
        ("s4 no_skip_bg",
         dict(scale=4, skip_bg=False)),
        ("s8 no_skip_bg",
         dict(scale=8, skip_bg=False)),

        # C) SVG feGaussianBlur
        ("s4 svg_blur=0.3",
         dict(scale=4, svg_blur=0.3)),
        ("s4 svg_blur=0.5",
         dict(scale=4, svg_blur=0.5)),

        # D) Combined best elements
        ("s8 core=0.45 eps=0.08 me=0.15",
         dict(scale=8, eps=0.08, max_error=0.15,
              iso_levels=(0.20, 0.45), iso_opacities=(0.50, 1.00))),
        ("s8 core=0.48 eps=0.10 me=0.15",
         dict(scale=8, eps=0.10, max_error=0.15,
              iso_levels=(0.20, 0.48), iso_opacities=(0.50, 1.00))),

        # E) Higher scale
        ("s12 eps=0.10 me=0.15",
         dict(scale=12, eps=0.10, max_error=0.15)),
        ("s16 eps=0.08 me=0.15",
         dict(scale=16, eps=0.08, max_error=0.15)),

        # F) Tuned sigma
        ("s8 sc=0.4 ss=0.8 eps=0.10 me=0.15",
         dict(scale=8, sc_sigma=0.4, ss_sigma=0.8, eps=0.10, max_error=0.15)),
        ("s8 sc=0.5 ss=1.0 eps=0.10 me=0.15",
         dict(scale=8, sc_sigma=0.5, ss_sigma=1.0, eps=0.10, max_error=0.15)),

        # G) Combos of everything
        ("s8 core=0.48 eps=0.10 no_skip_bg",
         dict(scale=8, eps=0.10, max_error=0.15, skip_bg=False,
              iso_levels=(0.20, 0.48), iso_opacities=(0.50, 1.00))),
        ("s8 raw_polygon no_skip_bg",
         dict(scale=8, use_raw_polygon=True, skip_bg=False)),
        ("s12 raw_polygon",
         dict(scale=12, use_raw_polygon=True)),
    ]
