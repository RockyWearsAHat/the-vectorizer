"""Test: compute soft field at upscale resolution (not just upscale the result).

Current approach: compute soft_raw at 1×, upscale to 4×.
New approach:     upscale distance maps to 4×, compute soft_raw at 4×.

The difference matters at cluster boundaries where the min() of other-cluster
distances creates non-linear kinks that get smoothed by bilinear upscaling.

Also test: tighter curve fitting, more clusters, uniform sigma.
"""
...
def precompute(image_bgr, num_k=24, merge_thresh=60.0):
    ...
    # Per-cluster distance map at original resolution
...
def run_upscale_v1(pre, *, scale=4, sc_sigma=0.6, ss_sigma=1.5,
                   iso_levels=(0.20, 0.50), iso_opacities=(0.50, 1.00),
                   eps=0.15, max_error=0.2, corner=60.0):
    """V1: Upscale the pre-computed soft field (original approach)."""
    h, w, K = pre['h'], pre['w'], pre['K']
    dist_map = pre['dist_map']
    ...
    for ci in pre['order']:
        if ci == pre['bg_cluster']:
            continue
        # Compute soft field at 1× resolution
        d_k = dist_map[:, :, ci]
        ...
        # Upscale AFTER soft field computation
...
def run_upscale_v2(pre, *, scale=4, sc_sigma=0.6, ss_sigma=1.5,
                   iso_levels=(0.20, 0.50), iso_opacities=(0.50, 1.00),
                   eps=0.15, max_error=0.2, corner=60.0):
    """V2: Upscale distance maps FIRST, compute soft field at upscale resolution.

    Key difference: the min() operation over other-cluster distances happens
    at 4× resolution, preserving boundary precision.
    """
    h, w, K = pre['h'], pre['w'], pre['K']
    dist_map = pre['dist_map']
    ew_up = cv2.resize(pre['edge_weight'], (w * scale, h * scale), interpolation=cv2.INTER_LINEAR)

    # Upscale ALL distance maps once
    dist_up = np.empty((h * scale, w * scale, K), dtype=np.float32)
    for k in range(K):
        dist_up[:, :, k] = cv2.resize(dist_map[:, :, k], (w * scale, h * scale),
    ...
    for ci in pre['order']:
        if ci == pre['bg_cluster']:
            continue
        # Compute soft field at upscale resolution
        d_k = dist_up[:, :, ci]
        omask = np.ones(K, dtype=bool); omask[ci] = False
        d_other = np.min(dist_up[:, :, omask], axis=2)
        soft_raw = d_other / np.maximum(d_k + d_other, 1e-10)

        sc = cv2.GaussianBlur(soft_raw, (0, 0), sigmaX=sc_sigma * scale)
        ss = cv2.GaussianBlur(soft_raw, (0, 0), sigmaX=ss_sigma * scale)
        soft = ew_up * sc + (1 - ew_up) * ss
...
if __name__ == "__main__":
    ...
    configs = [
        # --- V1 vs V2 at default params ---
        ("V1 s4 default",
         run_upscale_v1, dict(scale=4)),
        ("V2 s4 default (dist upscale)",
         run_upscale_v2, dict(scale=4)),
        ("V2 s4 sc0.8 ss0.8",
         run_upscale_v2, dict(scale=4, sc_sigma=0.8, ss_sigma=0.8)),

        # --- V2 with tighter curve fitting ---
        ("V2 s4 eps=0.05 me=0.1",
         run_upscale_v2, dict(scale=4, eps=0.05, max_error=0.1)),
        ("V2 s4 eps=0.10 me=0.15",
         run_upscale_v2, dict(scale=4, eps=0.10, max_error=0.15)),

        # --- V2 with uniform sigma (no dual) ---
        ("V2 s4 uniform σ=0.6",
         run_upscale_v2, dict(scale=4, sc_sigma=0.6, ss_sigma=0.6)),
        ("V2 s4 uniform σ=0.8",
         run_upscale_v2, dict(scale=4, sc_sigma=0.8, ss_sigma=0.8)),

        # --- V2 at higher scale ---
        ("V2 s6 default",
         run_upscale_v2, dict(scale=6)),
        ("V2 s8 default",
         run_upscale_v2, dict(scale=8)),

        # --- V2 with different halo ---
        ("V2 s4 h0.15@0.40",
         run_upscale_v2, dict(scale=4, iso_levels=(0.15, 0.50), iso_opacities=(0.40, 1.00))),
        ("V2 s4 h0.18@0.45",
         run_upscale_v2, dict(scale=4, iso_levels=(0.18, 0.50), iso_opacities=(0.45, 1.00))),
        ("V2 s4 h0.20@0.45",
         run_upscale_v2, dict(scale=4, iso_levels=(0.20, 0.50), iso_opacities=(0.45, 1.00))),

        # --- V2 s4 tighter fit + tuned halo ---
        ("V2 s4 eps=0.05 me=0.1 h0.15@0.40",
         run_upscale_v2, dict(scale=4, eps=0.05, max_error=0.1,
